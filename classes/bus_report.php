 ...
 */
 class bus_report {
	...
	function get_scheduled_courses() {
		...
		$where_clause .= ')';
		If ($counter>0)$where_clause = ' and '.$where_clause;
	...
	function update_totals($course, $registered, $waiting_list, $cancelled, $attended, $passed, $failed) {
		$this->total_courses[$course]++;
		$this->total_registered[$course] += $registered;
		$this->total_waiting_list[$course] += $waiting_list;
