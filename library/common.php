...
function calcCommonQnty($qnty, $unit)
{
	$sql = "Select value 
			From tbl_units
			Where unit='$unit'";
...
function do_consumeInventory($catvid, $amt)
{
        $catArr = explode("-", $catvid);


        // echo 'catid='.$catArr[0]." amt=".$amt;
        $catid = $catArr[0];

        $qsql = "Select level, ctype, lower
                        From tbl_inventory iv, tbl_categories ct
                        Where  iv.cid=ct.cid and iv.cid=$catid";

        $qresult=dbQuery($qsql);
        $qrow = dbFetchAssoc($qresult);
        extract($qrow);
        ...
        if ($level - $amt <= $lower)
        {
                $psql = "select prio from tbl_shoplist where cid = $catid";
                $presult = dbQuery($psql);


                if (mysqli_num_rows($presult) == 0)
                {
                        $isql = "Insert into tbl_shoplist (cid) values ($catid)";
                }
                else
                {
...
{	
	extract($row);
	do_consumeInventory($cid, $level*$reduce_percent/100);
}
