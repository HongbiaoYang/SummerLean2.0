												...
												<td colspan="6">
													<table width="100%">
														<tr>
															...
															<td class="right">
																<span class="head">Pass/Fail</span>
															</td>
															<td class="right">
														...
														foreach ($users as $q => $r) {
															...
															$user->set_profile();
															if ($_GET['action'] == 'edit' && $user->id == $_GET['user_id']) {
															?>
																<tr>
																	<form name="report_update" action="reporting.php?user_id=<?php echo $_GET['user_id'];?>&calendar_id=<?php echo $_GET['calendar_id'];?>" method="post">
																	<td class="right" width="25%"><?php echo $user->firstname.' '.$user->lastname; ?></td>
																	<td class="right">
																		<select name="attended" class="textbox1">
																			<option value=1 <?php if ($report->attended == 1) echo "SELECTED"; ?>>Yes</option>
																			<option value=2 <?php if ($report->attended == 2) echo "SELECTED"; ?>>No</option>
																			<option value=0 <?php if (!$report->attended) echo "SELECTED"; ?>>N/A</option>
																		</select>
																	</td>
																	<td class="right">
																		<select name="pass_fail" class="textbox1">
																			<option value=1 <?php if ($report->pass) echo "SELECTED"; ?>>Pass</option>
																			<option value=2 <?php if ($report->fail) echo "SELECTED"; ?>>Fail</option>
																			<option value=0 <?php if (!$report->pass && !$report->fail) echo "SELECTED"; ?>>N/A</option>
																		</select>
																	</td>
																	<td class="right"><input name="mark" type="textbox" value="<?php echo $report->mark; ?>" size="4" class="textbox1"></td>
																	<td class="right"><input name="comment" type="textbox" value="<?php echo $report->comment; ?>" class="textbox1" size="13"></td>
																	<td class="right"><input name="report_update" type="submit" value="Update" class="submit3"></td>
																	</form>
																</tr>
															<?php
															}
															...
															?>
																<tr>
																	...
																	<td class=<?php echo $class; ?>><?php if ($report->attended == 1) echo 'Yes'; else if ($report->attended == 2) echo 'No'; else echo 'N/A'; ?></td>
																	<td class=<?php echo $class; ?>><?php if ($report->pass) echo 'Pass'; elseif ($report->fail) echo 'Fail'; else echo 'N/A'; ?></td>
																	<td class=<?php echo $class; ?>><?php if ($report->mark) echo $report->mark; else echo 'N/A'; ?></td>
																	<td class=<?php echo $class; ?>><?php if ($report->comment) echo $report->comment; else echo 'N/A'; ?></td>
																	<td class=<?php echo $class; ?>><a href="reporting.php?action=edit&user_id=<?php echo $user->id; ?>&calendar_id=<?php echo $scheduled->calendar_id; ?>#Report">Edit</a></td>
																</tr>
															<?php
															}
														}
