...
*/
	...
	if (isset($_POST['add_jobs']) && tep_validate_entry($_POST['name'])) {
		...
		if (tep_validate_user($_SESSION['admin_user'], $access_level)) {
			...
			tep_db_perform(TABLE_JOBS, $sql_array);
		}
		else
		{
			$access_error = true;
	...
	switch ($action) {
		case 'delete': if (tep_validate_user($_SESSION['admin_user'])) {
			       		tep_delete_entry(TABLE_JOBS, $_GET['id']);
			       }
			       else
			       {
			       		$access_error = true;
					...
					<tr>
						<td>
							<table width='100%'>
								...
								<tr>
									<td align="left">
										<h4>Current Job Titles</h4>
										<table width="100%">
										<?php
											//display current centers
											$job_array = tep_get_jobs();
											if (empty($job_array)) {
											?>
												<tr>
													<td class="right">
													<?php
														echo 'There are no job titles registered';
													?>
													</td>
												</tr>
												<?php
											...
											else
											{
												foreach ($job_array as $s => $t) { ?>
													<tr>
														...
														<td class="left">
															<a href="edit_jobs.php?id=<?php echo $t['id']; ?>">Edit</a>&nbsp;|&nbsp;<a href="add_jobs.php?action=delete&id=<?php echo $t['id'] ?>">Delete</a>
														</td>
													</tr>
												<?php
												}
										...
										<form action="add_jobs.php" name="add_jobs" method="POST">
										<table width="100%">
											<tr>
												<td width="30%" class="left">
													Job Title:
											...
											<tr>
												<td colspan="2" align="center">
													<input name="add_jobs" type="submit" value="Add Job Title" class="submit3">
												</td>
											</tr>
										</table>
										</form>
