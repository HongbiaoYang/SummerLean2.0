...
function tep_get_courses($where_clause = '') {
	if ($where_clause == '') {
		$query = "select * from courses order by center, type, category, name";
	}
	else
	{
		$query = "select * from courses where ".$where_clause." order by center, type, category, name";
	}
	$result = tep_db_query($query);
	while ($row = tep_db_fetch_array($result)) {
		$course_array[] = array("id" => $row['id'],
				        "name" => $row['name'],
