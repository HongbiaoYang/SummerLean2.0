...
function tep_get_admin_users($search_term = '') {
	...
	{
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.admin='1' order by u.center, u.type, p.lastname";
	}
...
function tep_get_instructor_users($search_term = '') {
	if (strlen($search_term) > 0) {
		$query = "select * from users as u left join profiles p on (u.id = p.user_id) where u.instructor='1' and (u.username LIKE '%$search_term%' OR p.firstname LIKE '%$search_term' OR p.lastname LIKE '%$search_term') ORDER by u.username";
	}
	else
	{
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.instructor='1' order by p.lastname";
	}
	$result = tep_db_query($query);
	while ($row = tep_db_fetch_array($result)) {
...
function tep_get_users($search_term = '') {
	if (strlen($search_term) > 0) {
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.admin='0' and u.instructor='0' and (u.username LIKE '%$search_term%' OR p.firstname LIKE '%$search_term%' OR p.lastname LIKE '%$search_term%') order by p.lastname";
	}
	else
	{
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.admin='0' and u.instructor='0' order by p.lastname";
	}
	$result = tep_db_query($query);
	while ($row = tep_db_fetch_array($result)) {
...
function tep_get_users_instructors($search_term = '') {
	if (strlen($search_term) > 0) {
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.admin='0' and (u.username LIKE '%$search_term%' OR p.firstname LIKE '%$search_term%' OR p.lastname LIKE '%$search_term%') order by p.lastname";
	}
	else
	{
		$query = "select * from users as u left join profiles as p on (u.id = p.user_id) where u.admin='0' order by p.lastname";
	}
	$result = tep_db_query($query);
	while ($row = tep_db_fetch_array($result)) {
		$user_array[] = array('id' => $row['id'],
...
function tep_get_instructors() {
	$query = "select u.id, p.firstname from users as u left join profiles as p on (u.id = p.user_id) where u.instructor = '1' ORDER BY p.lastname";
	$result = tep_db_query($query);
	while ($row = tep_db_fetch_array($result)) {
		$instructor_id_array[] = array("id" => $row['id']);
	}
...
function tep_get_group_instructors($group_id) {
	...
	if (tep_db_num_rows($result) > 0) {
		while ($row = tep_db_fetch_array($result)) {
			$instructor_id_array[] = array("id" => $row['user_id']);
		}
	}
	else{
