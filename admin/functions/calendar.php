				...
				if (count($calendar_entry_array) > 0) {
					foreach ($calendar_entry_array as $g => $h) {
						if (isset($_GET['action']) && isset($_GET['id']) && strlen($_GET['action']) > 0) {
							if ($_GET['action'] == 'register' && $h['cancelled'] != 1) {
								$calendar .= '<br><a href="course_register.php?id='.$h["id"].'&user_id='.$_GET['id'].'">'.$h['name'].'</a>';
							}
							else if ($_GET['action'] == 'register' && $h['cancelled'] == 1) {
								$calendar .= '<br><font color="red">'.$h['name'].'</font>';
							}
							else
							{
								$calendar .= '<br><a href="course_registrations.php?id='.$h["id"].'">'.$h['name'].'</a>';
							}
						}
						else
						{
							if ($h['cancelled']) {
								$calendar .= '<br><a href="course_registrations.php?id='.$h["id"].'"><font color="red">'.$h['name'].'</font></a>';
							}
							else
							{
								$calendar .= '<br><a href="course_registrations.php?id='.$h["id"].'">'.$h['name'].'</a>';
